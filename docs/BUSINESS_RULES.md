...
|-------------|-----------|
| На кубах/сферах **нет текстур штрихкодов** | `pyzbar` нечего читать на SCAN LINE |
| WMS не подключён к боевому API Ozon | Используется mock `routes.yaml` |
| Для работающей ленты нужен любой маршрут | Включён **демо-fallback**: `by_class` напрямую ведёт в рукава |

**Временная демо-конфигурация** (не прод-политика):

```yaml
by_class:
  box: chute_a      # только чтобы кубы уезжали влево без EAN
  sphere: chute_b   # сферы — вправо (геометрия актуатора)
```

Это **технический компромисс симулятора**, а не утверждение «коробка = Сибирь».  
На защите говорим: *«В симуляторе нет наклеек — показываем fallback по CV; на видео с кодами и на проде главный — штрихкод WMS»*.
...
| Шаг | Что сделать |
|-----|-------------|
| P0 | Текстуры EAN/QR на объектах PyBullet → маршрут по `by_barcode_prefix` |
