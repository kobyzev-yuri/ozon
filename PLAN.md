...
> Статус: **скелет v0.1** в репозитории (`src/sorter/`, `ARCHITECTURE.md`). Полная постановка — 2 июля 2026, 11:00 МСК.  
> Установка дополнительных пакетов и выбор подхода — **после** появления условий.
...
|------|---------------------|-------------------|
| **Штрихкод** | основной для посылок / тар | возможно, если коды на макетах |
| **CV (класс / тип)** | ОВХ, контроль, fallback | вероятно по формулировке ТЗ |
| **Гибрид** | штрихкод + CV при сбое | самый реалистичный для эмуляции |

### Маршрутизация для эмуляции

**Не «нейросеть решает зону»**, а **lookup после идентификации**:

```yaml
# routes.yaml — эмуляция WMS
"4601234567890": zone_A    # barcode → зона
"box_small":       zone_B    # CV-класс → зона
default:           zone_reject
```
